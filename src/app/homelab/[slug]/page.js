...
export default async function HomelabDetailPage({ params }) {
  const { slug } = await params;

  const query = `*[_type == "homelab" && slug.current == $slug][0]`;
      ...
      image: ({ value }) => {
        ...
        return (
          <img
            ...
            style={{ borderRadius: '0.75rem', margin: '2rem auto', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', maxHeight: '600px', objectFit: 'cover', width: '100%' }}
          />
        );
      },
    },
  ...
  return (
    <main className="projects-container">
      <article className="article-card">
        ...
        <Link href="/homelab" className="back-link">
        ...
        <h1 className="article-title" style={{ marginBottom: '2rem' }}>{post.title}</h1>
